...
st.markdown("""
### 💡 使用指南：
1. **同时上传文件**：点击下方的上传框，按住 `Ctrl` 键**同时选中**你的广告表和产品信息表上传。
2. **多重条件筛选**：系统会自动按**创建时间最新排序**，并筛选 **`版本名称` 为 '优化组版本-XJP' 或 '优化组版本-GPTJ'** 且 **`状态 == '正常'`** 的数据。
...
def process_dataframe(df, file_name):
    ...
    # 2. 优化筛选版本名称：支持 '优化组版本-XJP' 或 '优化组版本-GPTJ'
...
if st.button("🚀 开始匹配并生成表格", type="primary"):
    if not uploaded_files or len(uploaded_files) < 2:
        ...
        with st.spinner("正在自动识别表格并处理中..."):
            ...
            if df_lp is None:
                st.error("❌ 未能识别到【产品信息表】，请检查是否包含 产品链接列。")
            ...
            if df_adv is not None and df_lp is not None:
                ...
                # 更新目标列和顺序：将“正文标题”拆回“正文”和“标题”，总共 9 列
                target_columns = ['真实SKU', '虚拟SKU', '国家', '产品链接', '封面链接', '素材链接', '正文', '标题', '描述']
                ...
                # 保存并提供下载
                output_path = "matched_result_v3.xlsx"
